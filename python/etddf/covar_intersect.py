...
def covariance_union(a,b,A,B):
    """
    Covariance union between disparate but confident estimates (a,A) and (b.B)
    """
    # correct shape of input means
    ...
    def U_fxn(S,V,D):
        ...
        for i in range(0,D.shape[0]):
            if D[i] > 1:
                ...
                max_vec.append(1)
        max_mat = np.diag(np.array(max_vec))

        return S.T @ V @ max_mat @ V.T @ S

    U_i = lambda x,P,m_cu: P + (m_cu-x)@((m_cu-x).T)

    def f(x,a,b,A,B,return_flag):
        ...
        # compute upper-triangular cholesky (numpy fxn returns LT)
        try:
            S = np.linalg.cholesky(U2).T
        except np.linalg.LinAlgError:
            pudb.set_trace()

        D, V = np.linalg.eig(np.linalg.inv(S).T @ U1 @np.linalg.inv(S))
        D = D.astype(np.float_)
        assert(D.dtype is not np.dtype('complex128'))
    ...
    args = (np.copy(a),np.copy(b),np.copy(A),np.copy(B),False)
    soln = minimize(f,np.copy(a),args)

    m_cu = soln.x

    if np.iscomplex(m_cu).any():
        m_cu = m_cu.astype(np.float_)

    U = f(m_cu,a,b,A,B,True)

    if np.iscomplex(U).any():
        U = U.astype(np.float_)

    return m_cu, U

def covariance_union_simple(a,b,A,B):
    """
    Simplied optimization problem for solving covariance union.
    """
    # correct shape of input means
    a = np.reshape(a,(a.shape[0],1))
    b = np.reshape(b,(b.shape[0],1))
    ...
    def f(omega):
        ...
        Ua = A + omega**2 * (c @ (c.T))
        Ub = B + (1-omega)**2  * (c @ (c.T)) 

        return np.trace(Ua) + np.trace(Ub)
