...
if j~=k
    plot(input_tvec,agents{j}.local_filter.state_history(4*(k_loc-1)+1,:) - agents{k}.true_state(1,:),'Color',color_wheel(1,:))
    plot(input_tvec,sigma*sqrt(squeeze(agents{j}.local_filter.cov_history(4*(k_loc-1)+1,4*(k_loc-1)+1,:))),'Color',color_wheel(1,:),'LineStyle','--')
    plot(input_tvec,-sigma*sqrt(squeeze(agents{j}.local_filter.cov_history(4*(k_loc-1)+1,4*(k_loc-1)+1,:))),'Color',color_wheel(1,:),'LineStyle','--','HandleVisibility','off')
...
%     legend(str1,strcat(num2str(sigma),'\sigma'),strcat('-',num2str(sigma),'\sigma'),str2,strcat(num2str(sigma),'\sigma'),strcat('-',num2str(sigma),'\sigma'),strcat('cent. KF ',num2str(j)),strcat('cent. KF ',num2str(k)))
    ...
    plot(input_tvec,agents{k}.local_filter.state_history((j_loc-1)*4+1,:)-agents{j}.true_state(1,:),'Color',[1 0 0])
    plot(input_tvec,sigma*sqrt(squeeze(agents{k}.local_filter.cov_history((j_loc-1)*4+1,(j_loc-1)*4+1,:))),'Color',[1 0 0],'LineStyle','--')
    plot(input_tvec,-sigma*sqrt(squeeze(agents{k}.local_filter.cov_history((j_loc-1)*4+1,(j_loc-1)*4+1,:))),'Color',[1 0 0],'LineStyle','--','HandleVisibility','off')
